...
# ---------- 4. 企业查询（全年代折线 + 红星 + 1 屏紧凑） ----------
@app.route('/query')
def query():
    code = request.args.get('code', '').strip()
    name = request.args.get('name', '').strip()
    year = request.args.get('year', '').strip()  # 可选
    ...
    # ① 压缩到 1 屏：宽度 100% + 高度紧凑 + 字体缩小
    fig, ax = plt.subplots(figsize=(10, 3.5))  # 宽度适中，高度压缩
    ...
    ax.set_xticklabels(trend.index, rotation=45, ha='right', fontsize=9)
...
def industry():
    ...
    if year:
        ...
        df[col_comp] = pd.to_numeric(df[col_comp], errors='coerce')
        df[col_norm] = pd.to_numeric(df[col_norm], errors='coerce')
        df = df.sort_values(col_comp, ascending=False)   # 全表，不截断

        # 前20折线
        df20 = df.head(20)
        plt.rcParams['font.family'] = 'SimHei'; plt.rcParams['axes.unicode_minus'] = False
        width_inch = max(6, len(df20) * 0.5)
        fig, ax = plt.subplots(figsize=(width_inch, 2.8))
...
def industry_bar():
    ...
    df[col_comp] = pd.to_numeric(df[col_comp], errors='coerce')
    df[col_norm] = pd.to_numeric(df[col_norm], errors='coerce')
    df = df.sort_values(col_comp, ascending=False).head(20)  # 前20

    plt.rcParams['font.family'] = 'SimHei'; plt.rcParams['axes.unicode_minus'] = False
    # ① 超紧凑：宽度 = 企业数 × 0.5 英寸（约 1.27 cm/企）+ 超紧边距
    width_inch = max(6, len(df) * 0.5)  # 最少 6 英寸，每多 1 企 +0.5
    fig, ax = plt.subplots(figsize=(width_inch, 2.8))  # 高度也压到 2.8
    ...
    ax.set_xlabel("企业", fontsize=8); ax.set_ylabel("指数", fontsize=8); ax.grid(alpha=0.3)
    # 超密刻度：每 3 个企业显示 1 个标签，保证不重叠
    step = max(1, len(df) // 10)  # 10 个标签以内
    ax.set_xticks(range(0, len(df), step))
    ax.set_xticklabels(df['企业名称'].iloc[::step], rotation=45, ha='right', fontsize=7)
    ax.legend(fontsize=8)
    fig.tight_layout(pad=0.3)  # 超紧边距
    buf = io.BytesIO(); fig.savefig(buf, format='png', dpi=120); buf.seek(0)
    img = base64.b64encode(buf.read()).decode(); plt.close()
